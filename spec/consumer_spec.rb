...
describe Responsible::Consumer do
  context "#can_see?" do
    ...
    it "return true if any of the restriction are in the valid list" do
      expect(with_no_restrictions.can_see?(%i[even prime])).to eq(false)

      expect(with_one_restrictions.can_see?(%i[even prime])).to eq(true)
      expect(with_two_restrictions.can_see?(%i[even prime])).to eq(true)
    end
