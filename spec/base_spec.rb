...
describe Responsible::Base do
  context "when a simple property is declare" do
    ...
    context "when 'to' parameter passed in" do
      it "raises and error if to method is not defined" do
        ...
        expect(klass.new(consumer, data_class.new).delegated).to eq(true)
      end
    end
  end
  ...
  describe "#as_json" do
    ...
    it "allows to delegate to hash key" do
      klass = Class.new(described_class) do
        property :no_role, delegate: :hash_key
      end
    ...
    it "allows to delegate to hash key with different name" do
      klass = Class.new(described_class) do
        property :no_role, delegate: :hash_key, to: :no_role?
      end
